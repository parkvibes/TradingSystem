...
class StockBrokerApplication:
    ...
    def select_stock_broker(self, stock_broker: StockDriver):
        self.__stock_broker = stock_broker

    def get_broker(self):
        return self.__stock_broker

    def login(self, _id: str, pw: str) -> None:
        self.__stock_broker.login(_id, pw)

    def purchase(self, stock_code: int, price: int, amount: int) -> None:
    ...
    def sell_nice_timing(self, stock_code, amount):
        prev_price = self.__stock_broker.get_price(stock_code)
        time.sleep(self.__sleep_ms_between_tries / 1000)
        cur_price = self.__stock_broker.get_price(stock_code)

        if prev_price > cur_price:
            self.__stock_broker.sell(stock_code, cur_price, amount)
            return cur_price * amount
